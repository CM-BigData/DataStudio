...
<p align="center">
  ...
  Development Guide · Clean data and route samples for retention, removal, or review.
</p>
...
- LLM/VLM/OCR/CLIP/ASR: configure `*_API_KEY`, `*_API_BASE`, `*_MODEL`, and endpoint path variables based on [`configs/api.env.example`](configs/api.env.example), then reference those environment variable names in operator `params`.
