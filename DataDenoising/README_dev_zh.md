...
- LLM/VLM/OCR/CLIP/ASR：在 [`configs/api.env.example`](configs/api.env.example) 对应变量基础上配置 `*_API_KEY`、`*_API_BASE`、`*_MODEL` 和 endpoint path，并在算子 `params` 中引用对应环境变量名。
