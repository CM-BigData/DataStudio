...
class TextDatasetScoreOperator(BaseOperator):
    operator_name: str = "text_dataset_score"  # Registered operator name used to aggregate text issues and score samples.

    def process(self, item: dict[str, Any]) -> dict[str, Any]:
        """Calculate a sample score and handling suggestion from text issues.
        ...
        Args:
            item (dict[str, Any]): Text sample that already finished quality checks.

        Returns:
            dict[str, Any]: Sample with `score`, `level`, `action`, and `suggestions` written back.

        Examples:
            >>> sample = {"issues": ["empty_text"]}
            >>> TextDatasetScoreOperator({}).process(sample)["action"]
            'drop'
        """
