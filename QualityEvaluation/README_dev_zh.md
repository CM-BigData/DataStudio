...
QualityEvaluation 是一个基于 workflow 的轻量级数据质量评测工具，包名为 `quality-eval-workflow-engine`，命令入口为 `quality-eval`。当前实现面向文本和图像数据集，支持通过 YAML/JSON workflow 调用端到端算子，对样本进行结构、完整性、长度、异常字符、重复、图片格式、分辨率、清晰度、空白图、疑似 AI 生成图、标注 bbox 等质量检查，并输出样本级结果、汇总 JSON、Markdown 报告、算子追踪日志、checkpoint 和错误队列。
